package com.xfarmer.common.util.redis;
...
public class RedisUtil {
    ...
    public void del(String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                ...
                List<String> collect = Arrays.stream(key).map(item -> PREFIX + item).collect(Collectors.toList());
    ...
    @SuppressWarnings("unchecked")
    public void delNotPrefix(String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                redisTemplate.delete(key[0]);
            } else {
                List<String> collect = Arrays.stream(key).collect(Collectors.toList());
                redisTemplate.delete(collect);
            }
        }
    }
     ...
     * @return
     */
    public Set<String> batchKeys(String key){
        String prefix1 = PREFIX +key + "*";
        // 获取所有的key
        Set<String> keys = redisTemplate.keys(prefix1);
        return keys;
    }
    ...
    /**
     * 模糊查询key的所有值（*号自动添加结尾）
     * @param key 需要模糊查询的key
     * @return
     */
    public List<Object> batchKey(String key){
        Set<String> keys = batchKeys(PREFIX +key);
        // 批量获取数据
