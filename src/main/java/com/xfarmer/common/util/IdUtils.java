...
public class IdUtils {
    private static String middle = "xfarmer";
    ...
    static {
        ...
        middle = MathUtils.makeUpNewData(Math.abs(NetworkUtils.getHostIp().hashCode() != 0 ? NetworkUtils.getHostIp().hashCode() : -1) + "", 4) +
                MathUtils.makeUpNewData(NetworkUtils.getPid(), 4);
    }


    public static String getMessageCode(int length) {
        return MathUtils.randomDigitNumber(length);
    }
...
//                middle +
    ...
    private static class DigestUtils {
        ...
        public static String md5Hex(String str, boolean isUpper) throws MyException {
            try {
    ...
    private static class MathUtils {
        ...
        public static String makeUpNewData(String target, int length, String add) {
            ...
            if (target.startsWith(CONNECTOR)) {
                newTarget = target.replace(CONNECTOR, "");
            }
            if (newTarget.length() >= length) {
                return newTarget.substring(0, length);
            }
            StringBuilder sb = new StringBuilder(FIRST_DEFAULT_DIGITS);
            for (int i = 0; i < length - (1 + newTarget.length()); i++) {
                sb.append(add);
            }
            return sb.append(newTarget).toString();
        }
